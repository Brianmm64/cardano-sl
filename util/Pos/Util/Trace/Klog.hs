...
import           Pos.Util.Log (LogContext, LoggerName, logMessage)
{-
...
klogTrace = Trace $ Op $ \namedLogItem ->
    let privacy = liPrivacy (lnItem namedLogItem)
     ...
     in case privacy of
            Private -> logMessage {-loggerName-} severity message --selectPrivateLogs
            Public  -> logMessage {-loggerName-} severity message --selectPublicLogs
            Both    -> logMessage {-loggerName-} severity message --selectBoth
--   where
--     selectPrivateLogs :: LogHandler -> Bool
--     selectPrivateLogs = not . selectPublicLogs
--     selectPublicLogs :: LogHandler -> Bool
--     selectPublicLogs p = ".pub" `isSuffixOf` p
--     selectBoth :: LogHandler -> Bool
